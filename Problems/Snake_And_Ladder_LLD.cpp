...
class Board{
    private:
        int finalPosition;
        unordered_map<int, int> mp; //position->final
        unordered_map<string, int> playz; //player->position
    public:

        Board(int f, vector<pair<int,int>> snakes, vector<pair<int,int>> ladders, vector<string> players) : finalPosition(f) {
            for(auto &it: snakes){
        ...
        bool diceRolled(int num, string player){
            auto it1= playz.find(player);
            int x = (*it1).second + num;
            if(x>finalPosition){
                cout<< player + " rolled a " + to_string(num) + " and moved from " + to_string((*it1).second) + " to " + to_string((*it1).second)<<endl;
                return 0;
            }
            while(mp.find(x) != mp.end()){
                x = (*(mp.find(x))).second;
            }
            cout<< player + " rolled a " + to_string(num) + " and moved from " + to_string((*it1).second) + " to " + to_string(x)<<endl;
            playz[player] = x;
            if(x == finalPosition){
...
int main(){
    ...
    int snakeNum, ladderNum, playerNum;

    cin>>snakeNum;
    vector<pair<int,int>> snakes(snakeNum); //head>tail
    for(auto &it:snakes){
        cin>>it.first;
        cin>>it.second;
    }
    ...
    cin>>ladderNum;
    vector<pair<int,int>> ladders(ladderNum);   //start<end
    for(auto &it:ladders){
        cin>>it.first;
        cin>>it.second;
    }
    ...
    cin>>playerNum;
    vector<string> players(playerNum);
    for(auto &it:players){
        cin>>it;
    }

    Board* B1 = new Board(finalPosition, snakes, ladders, players);
    while(true){
        bool res = 0;
        for(auto &it:players){
            int x = rand()%6 + 1;
            
            res = B1->diceRolled(x,it);
            if(res){
                break;
            }
        }
